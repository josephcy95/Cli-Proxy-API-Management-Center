...
describe('Claude Fable quota', () => {
  ...
  test('ignores malformed and unrelated limits while preserving standard windows', () => {
    ...
    // Standard named windows always appear. Non-Fable scoped limits from limits[]
    // are also surfaced (forward-compatible). Invalid Fable candidates are ignored.
    const byId = Object.fromEntries(windows.map((w) => [w.id, w.usedPercent]));
    expect(byId['five-hour']).toBe(10);
    expect(byId['seven-day']).toBe(20);
    expect(byId['seven-day-fable']).toBeUndefined();
    expect(byId['limit-weekly_scoped-1']).toBe(35); // Sonnet scoped
    expect(byId['limit-session-2']).toBe(50);
  });
});
